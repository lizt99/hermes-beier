...
import { coerceValue, parseKeyValueToken, shouldPromoteSelectorToCommand, } from './call-argument-values.js';
...
export function parseCallArguments(args) {
    ...
    const flagState = { coercionMode: 'default' };
...
function scanCallTokens(args, result, state) {
    const positional = [];
...
function applyLeadingCallExpression(positional, result) {
...
function resolveSelectorAndTool(positional, result, callExpressionProvidedServer, callExpressionProvidedTool) {
...
function applyTrailingArguments(positional, result, state) {
...
function appendLiteralPositionalArguments(literalPositional, result, state) {
...
function handleServerFlag(context) {
...
function handleTailLogFlag(context) {
    context.result.tailLog = true;
    return context.index + 1;
}
function handleSaveImagesFlag(context) {
    context.result.saveImagesDir = consumeFlagValue(context.args, context.index, '--save-images', '--save-images requires a directory path.');
    return context.index + 2;
}
function handleNoopFlag(context) {
    return context.index + 1;
}
function handleRawStringsFlag(context) {
    context.state.coercionMode = 'raw-strings';
    context.result.rawStrings = true;
    return context.index + 1;
}
function handleNoCoerceFlag(context) {
    context.state.coercionMode = 'none';
    context.result.rawStrings = true;
    return context.index + 1;
}
function handleArgsFlag(context) {
    ...
    try {
        ...
        throw new Error(`Unable to parse --args: ${error.message}`, { cause: error });
    }
...
function consumeFlagValue(args, index, token, missingValueMessage) {
