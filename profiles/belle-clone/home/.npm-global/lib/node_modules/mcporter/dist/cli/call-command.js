...
import { CALL_HELP_ADHOC_SERVER_LINES, CALL_HELP_ARGUMENT_LINES, CALL_HELP_EXAMPLE_LINES, CALL_HELP_RUNTIME_FLAG_LINES, } from './call-help.js';
...
import { chooseClosestIdentifier, normalizeIdentifier, renderIdentifierResolutionMessages, } from './identifier-helpers.js';
...
export async function handleCall(runtime, args) {
...
async function prepareCallRequest(runtime, args) {
    ...
    const schemaAwareArgs = await enforceSchemaStringTypes(runtime, server, tool, hydratedArgs, parsed.schemaStringCoercionCandidates, timeoutMs);
    return { parsed, server, tool, hydratedArgs: schemaAwareArgs, timeoutMs };
}
async function normalizeParsedCallArguments(runtime, parsed) {
    let ephemeralSpec = parsed.ephemeral ? { ...parsed.ephemeral } : undefined;
    const nameHints = [];
    const absorbUrlCandidate = (value) => {
        if (!value) {
            return value;
        }
...
async function resolveServerAndTool(runtime, parsed) {
...
async function invokePreparedCall(runtime, prepared) {
    let invocation;
    try {
        invocation = await invokeWithAutoCorrection(runtime, prepared.server, prepared.tool, prepared.hydratedArgs, prepared.timeoutMs);
    }
    catch (error) {
...
function renderCallResult(result, parsed) {
...
async function maybeDescribeServer(runtime, server, tool, outputFormat) {
    if (tool === 'list_tools') {
...
function resolveCallTarget(parsed, options = {}) {
...
async function enforceSchemaStringTypes(runtime, server, tool, args, rawCandidates, timeoutMs) {
    if (!rawCandidates || Object.keys(rawCandidates).length === 0) {
        return args;
    }
    const tools = await withTimeout(loadToolMetadata(runtime, server, { includeSchema: true }), timeoutMs).catch(() => undefined);
    if (!tools) {
        return args;
    }
...
function schemaAllowsString(descriptor) {
...
async function hydratePositionalArguments(runtime, server, tool, namedArgs, positionalArgs) {
    ...
    if (positionalArgs.length > remaining.length) {
        throw new Error(`Too many positional arguments (${positionalArgs.length}) supplied; only ${remaining.length} parameter${remaining.length === 1 ? '' : 's'} remain on ${tool}.`);
    }
    const hydrated = { ...namedArgs };
...
async function inferSingleToolName(runtime, server) {
...
async function invokeWithAutoCorrection(runtime, server, tool, args, timeoutMs) {
...
async function attemptCall(runtime, server, tool, args, timeoutMs, allowCorrection) {
    try {
        ...
        if (error instanceof Error && error.message === 'Timeout') {
            ...
            throw new Error(`Call to ${server}.${tool} timed out after ${timeoutDisplay}. Override MCPORTER_CALL_TIMEOUT or pass --timeout to adjust.`, { cause: error });
        }
        if (!allowCorrection) {
            throw error;
        }
...
async function maybeResolveToolName(runtime, server, attemptedTool, error) {
    ...
    const resolution = chooseClosestIdentifier(attemptedTool, tools.map((entry) => entry.tool.name));
    if (!resolution) {
        return undefined;
    }
    return resolution;
}
function extractMissingToolFromError(error) {
...
function maybeReportConnectionIssue(server, tool, error) {
...
function summarizeIssueMessage(message) {
    if (!message) {
        return '';
    }
