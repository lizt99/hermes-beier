...
3️⃣ **【防限流与SEO核心规定】**：在生成 MDX 文本时，绝对禁止使用任何固定的文案模板！每次必须根据具体的医院和场景撰写原创内容。
      - **SEO标题限制（严格）**：标题必须**严格控制在 20 个字以内**！必须包含长尾SEO搜索词（如：[医院名称]+[具体科室]+护工/陪护）。
      - **正文SEO霸屏布局**：正文的**前50个字内**，必须高频且自然地埋入长尾搜索词（医院、具体科室、住院陪护等），吃透搜索流量。
...
- **指令**：“生成笔记配图” 或 哥哥提供现场照片及护工照片要求生成图片
  ...
  👉 **必做前置步骤**：在调用脚本生成图片前，**必须先执行 `ls -la <绝对路径>/scripts/assets/`**，根据哥哥提供的核心词（如“曹双喜”、“女护工”、“西京医院”）模糊匹配并获取当前真实的完整文件名。绝不能凭记忆盲猜文件名，否则会导致脚本运行报错。
      👉 **必须动态指定行为逻辑，且兼顾文字与构图**：
          ...
          3. **避坑：要求近景/中景特写，同时拒绝死板摆拍！** 必须展现【真实服务抓拍感】（如手部正推着轮椅扶手、拿着检查报告单、端水等工作交互状态），但躯干必须保持基本直立，绝对禁止过度弯腰。禁止远景全身照，镜头必须拉近让背部文字足够大且平整清晰。**必须要求AI让护工呈现【微转身】或【背对镜头/侧背影】的姿态，背部正对画面中心，最大化马甲电话号码的视觉面积**，因为客户主要通过看图直接打电话，视觉占比越高转化越好。
