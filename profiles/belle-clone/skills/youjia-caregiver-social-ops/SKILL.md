...
- **指令**：“生成一篇关于 [XX] 的笔记” 
  👉 Belle 会自动生成包含丰富排版的腾讯智能文档 (Smartcanvas)。若哥哥明确要求带配图，或默认流程需要配图：
      1️⃣ 先使用 指令：“生成笔记配图” 例如 xxx医院 xxx护工 照片, 生成图片到本地。
      2️⃣ 调用 `upload_image` 工具获取 `image_id`。**注意：因为 `mcporter call` 有 CLI 参数长度限制，禁止通过终端命令行传递 base64**。必须使用 Python `httpx` 脚本直接请求 `https://docs.qq.com/openapi/mcp` 端点（鉴权 token 从 `~/.hermes/profiles/belle-clone/home/.mcporter/mcporter.json` 获取）。
      3️⃣ 在生成的 MDX 文本中插入 `<Image src='image_id值' />`（或 Markdown 的 `![图片](image_id值)`）。
...
- **指令**：“生成笔记配图” 或 哥哥提供现场照片及护工照片要求生成图片
  ...
  👉 执行示例：`python3 <绝对路径>/scripts/generate_image.py --scenes <场景图> --caregivers <护工图> --prompt "这是病房场景。动作要求：站在病床边陪伴，保持身体直立。务必拉近镜头，采用【近景腰部以上的半身照】，让护工背部占据画面主要比例（不要远景！），确保『优加陪护』和电话『15793592202』字足够大、极度清晰。" --count 1 --output <绝对路径>/scripts/results` 
